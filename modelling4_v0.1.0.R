...
for(a in c(4)){
  code = unique(ts_norm_clip$code)[a]
  message(code)
  ts_norm_trim = subset(ts_norm_clip2, code == unique(ts_norm_clip$code)[a])
  ...
  m2_output = ts_norm_trim %>% group_by(tips_id, site_spec) %>%
    do(fit_lm = lm(log(abundance_norm+0.01) ~ year, data = .))
  
  ts_norm_trim2 = NULL
  for(b in 1:nrow(m2_output)){
    ts_norm_trim2_tmp = data.frame(
      tips_id = m2_output$tips_id[b],
      site_spec = m2_output$site_spec[b],
      coef = coef(m2_output$fit_lm[[b]])[2],
      lci = confint(m2_output$fit_lm[[b]])[2,1],
      uci = confint(m2_output$fit_lm[[b]])[2,2]
    )
    ts_norm_trim2 = rbind(ts_norm_trim2, ts_norm_trim2_tmp)
  }
  
  ts_norm_trim$abundance_norm_log2 = ts_norm_trim$abundance_norm_log
  ts_norm_trim$abundance_norm_centre2 = ts_norm_trim$abundance_norm_centre
  
  tip_freq = as.data.frame(table(ts_norm_trim2$tips_id))
  colnames(tip_freq) = c("tips_id", "freq")
  tip_freq$tips_id = as.integer(tip_freq$tips_id)
  ts_norm_trim2 = left_join(ts_norm_trim2, tip_freq)
  ts_norm_trim2 = ts_norm_trim2[order(ts_norm_trim2$freq),]
  
  uni_sites = unique(ts_norm_trim2$site_spec)
  uni_sites_sample = sample(ts_norm_trim2$site_spec, round(length(uni_sites)/10))
  ts_norm_trim4 = ts_norm_trim
  ts_norm_trim4[ts_norm_trim4$site_spec %in% uni_sites_sample, c("abundance_norm_log2", "abundance_norm_centre2")] = NA
  
  
  message("   Model 1 - Intercept model")
  m1 = inla(abundance_norm_log2 ~ 
              year_centre + 
            ...
            data = ts_norm_trim4, family = "gaussian", 
  ...
  m2_res = ts_norm_trim4$abundance_norm_centre - m2_fit
  m2_pred = sd(m2_fit)^2
  m2_obv = bri.hyperpar.summary(m2)[2,4]
  m2_tip = bri.hyperpar.summary(m2)[3,4]
  m2_sit = bri.hyperpar.summary(m2)[4,4]
  m2_sig = bri.hyperpar.summary(m2)[1,4]
  
  
  
  
  ts_norm_trim4$site_spec2 = ts_norm_trim4$site_spec
  message("   Model 4 - Correlation model")
  m3 = inla(abundance_norm_centre2 ~ 
              year_centre +
  ...
  m3_res = ts_norm_trim4$abundance_norm_centre - m3_fit
  m3_pred = sd(m3_fit)^2
  ...
  m3_sit_h = bri.hyperpar.summary(m3)[7,4]
  m3_sit = bri.hyperpar.summary(m3)[8,4]
  ...
  saveRDS(output_list, paste0("data/produced_data/run_6corr7_cv4_", a,".rds"))
