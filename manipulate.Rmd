...
And remove population trends for species lacking phylogenetic information. This steps risks imposing a bias as species without phylogeny information may be more at risk of having a particular trend type (e.g. declining). This is something we are unable to assess in the current manuscript.
...
Next we remove population trends which contain zeros. These zeros may simply represent missed detections. But could also represent extreme cases like extinction or recolonisation, which likely possess a different process and can have accessive infleucne on models (as shown by Leung 2020. Clustered vs catastropic declines.  Nature. 

We also have opted to constrain the datasets, only keeping the 50% most longest time-series. In some cases, this still results in population time-series with only 2 abundance, so for all datasets - except the small CaPTrends and TimeFISH, which have less than 50,000 population time series - we also impose a further constraint, only keeping trends that contain 5 or more population estimates as this will improve the reliability of trends. In the datasets with less than 50000 populations, this further subsetting would have left the datasets with very few populations, and so was deemed undesirable.
...
# Data for modelling Fig 3

Here we prepare the BioTIME data to predict abundance trends for a given site. To make predicitons in INLA, you have to specify the data structure priori. This involves specifying the spatial site you wish to predict. All other data processing is identical to '#Data for modelling_core.R, modelling_sensitivity_structure.R, modelling_prediction.R'

```{r, tidy_data3, message = FALSE}
analysis_list = list()
for(a in unique(compiled_data$dataset_id)[1]){
  ...
  newdata <- tmp_df[c(1:100),]
  newdata[] = NA
  newdata$region_code = 56
  newdata$site_code = 336
  newdata$site_code2 = 336
  newdata$year_centre = seq(min(tmp_df$year_centre), max(tmp_df$year_centre), length.out = 100)
  tmp_df = rbind(tmp_df, newdata)
...
saveRDS(analysis_list, "../data/derived_data/analysis_list_predict.rds")
```
...
saveRDS(analysis_list, "../data/derived_data/analysis_list_predic2.rds")
```
