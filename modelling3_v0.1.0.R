...
for(a in c(10)){
  ...
  uni_sites = unique(ts_norm_trim$site_spec)
  uni_sites_sample = sample(uni_sites,(length(uni_sites)/2))
  ts_norm_trim4 = list()
  for(c in c(1:(length(uni_sites)))){
    tmp_ts_norm_trim4 = subset(ts_norm_trim, site_spec == uni_sites[c])
    if(uni_sites[c] %in% uni_sites_sample){
      tmp_ts_norm_trim4$abundance_norm_log2[c(nrow(tmp_ts_norm_trim4))]  = NA
      tmp_ts_norm_trim4$abundance_norm_centre2[c(nrow(tmp_ts_norm_trim4))]  = NA
    } else {
      
    }
    ts_norm_trim4[[c]] = tmp_ts_norm_trim4
  }
  ts_norm_trim4 = rbindlist(ts_norm_trim4)
  ...
  m1_res = log(ts_norm_trim4$abundance_norm+0.01) - m1_fit
  m1_pred = sd(m1_fit)^2
  ...
  descriptives = data.frame(
    code = code,
    n = nrow(ts_norm_trim4),
    n_species = length(unique(ts_norm_trim4$tips_id)),
    n_site = length(unique(ts_norm_trim4$site_id)),
    n_trends = length(unique(ts_norm_trim4$site_spec)) 
  )
  ...
  output_list[[7]] = uni_sites_sample
  output_list[[8]] = ts_norm_trim
  
  message("SUCCESS")
  
  
  saveRDS(output_list, paste0("produced_data/run_modelling3_", a,".rds"))
